    ...
    class SendHotelAlertEvent : IPacketWebEvent
    {
        public void Parse(WebClient Session, ClientPacket Packet)
        {
            ...
            if (!string.IsNullOrWhiteSpace(Url))
            {
                ...
                AkiledEnvironment.GetGame().GetClientWebManager().SendMessage(new NotifAlertComposer("comunicadostaff", "All'attenzione del comunicato del personale", Message, "Vai al collegamento!", 0, Url), Session.Langue);
                return;
            }
            ...
            if (EventAlert)
            {
                ...
                AkiledEnvironment.GetGame().GetClientWebManager().SendMessage(new NotifAlertComposer("alertajuego", "Notifica evento", Message, "Vai all'Evento!", Client.GetHabbo().CurrentRoom.Id, ""), Session.Langue);
            ...
            else
            {
                ...
                AkiledEnvironment.GetGame().GetClientWebManager().SendMessage(new NotifAlertComposer("alertastaff", "Messaggio dallo staff", Message, "Inteso !", 0, ""), Session.Langue);
            }
