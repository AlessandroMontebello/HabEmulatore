    ...
    public class GameClient
    {
        ...
        public async Task TryAuthenticateAsync(string AuthTicket)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    string last_success_login = "Il tuo ultimo accesso riuscito è stato ";
                    var last_success_login1 = last_success_login;
                    SendMessage(RoomNotificationComposer.SendBubble("last_success_login", last_success_login1 + new DateTime(1970, 1, 1, 0, 0, 0).AddSeconds(GetHabbo().LastOnline + 7200.0) + " ore."));
                    ...
                    if (!GetHabbo().Nuxenable)
                    {
                        ...
                        Console.WriteLine("Utente connesso: " + GetHabbo().Username + " nell'hotel, con IP :" + GetHabbo().IP + " ",
                                ...
                                embeds: new[]
                    {
                                ...
                                "Notifica di accesso" + DiscordEmoji.Thumbsup,
                                 color: 4833120,
                                author: new DiscordMessageEmbedAuthor(GetHabbo().Username),
                                description: "Il cliente dell'hotel è entrato",
                            ...
                            await client.SendToDiscord(message).ContinueWith(task => { if (task.IsFaulted) { Console.WriteLine("Errore nell'inviare il messaggio: " + task.Exception.Message); } }).ConfigureAwait(true);

                            Console.WriteLine("login inviato al Discord ", ConsoleColor.DarkCyan);
                    ...
                    {
                        SendNotification("<font color = '#B40404'><font><b>Congratulazioni!</b></font></font>\n\nVogliamo congratularci con te per essere un amico fedele, e per aver completato 20 ore di connessione e come ricompensa > 20000 ore in hotel, ti abbiamo regalato un Trono Classico..\r");
                        ...
                        foreach (Item PurchasedItem in Items)
                        {
                            if (GetHabbo().GetInventoryComponent().TryAddItem(PurchasedItem))
                            {
                                ...
                                SendPacket(RoomNotificationComposer.SendBubble("rewards", "Hai appena ricevuto un:\n\n " + premiox20horasname + ".\n\nWow, " + GetHabbo().Username + ", Controlla il tuo inventario, sembra che ci sia qualcosa di nuovo! \n\nCliacca qui", "inventory/open/furni"));
                                Console.ForegroundColor = ConsoleColor.DarkGreen;
                                Console.WriteLine("Il giocatore: " + GetHabbo().Username + " Ha ricevuto il premio per essere rimasto online per 20 ore. ", "Akiled.Users",
                                ConsoleColor.DarkGreen);
                            }
        ...
        private async Task IsNewUserAsync()
        {
            ...
            if (GetHabbo().NewUser && GetHabbo().Gender.ToUpper() != "M")
            {
                ...
                Console.ForegroundColor = ConsoleColor.DarkGreen;
                Console.WriteLine("Nuovo giocatore connesso: " + GetHabbo().Username + " in HabHub, con l'IP :" + GetHabbo().IP + " ", "Akiled.Users",
                ConsoleColor.DarkGreen);
                ...
                if (Webhook == "true")
                {
                    ...
                    var message = new DiscordMessage(
                     "La sicurezza è importante per noi " + DiscordEmoji.Grinning,
                        username: UserNameD,
            ...
            {
                                new DiscordMessageEmbed(
                                "Notifica nuovo utente" + DiscordEmoji.Thumbsup,
                                 color: 1824480,
                                author: new DiscordMessageEmbedAuthor(GetHabbo().Username),
                                description: "È entrato per la prima volta nel client dell'hotel",
                                thumbnail: new DiscordMessageEmbedThumbnail(Webhooka_avatar + GetHabbo().Look),
                                footer: new DiscordMessageEmbedFooter("Creato per "+UserNameD, ProfilePicture)
        )
            }
            );
                    await client.SendToDiscord(message);
                    Console.WriteLine("nuovo accesso utente inviato a Discord ", ConsoleColor.DarkYellow);
            ...
            if (GetHabbo().NewUser && GetHabbo().Gender.ToUpper() != "F")
            {
                ...
                Console.WriteLine("Nuovo accesso utente invitato a Discord " + GetHabbo().Username + "nell'Hotel, con l'IP :" + GetHabbo().IP + " ", "Akiled.Users",
                ConsoleColor.DarkGreen);
        ...
        public bool Antipub(string Message, string type, int RoomId = 0)
        {
            ...
            else if (PubCount == 4)
            {
                ...
                Task task = AkiledEnvironment.GetGame().GetClientManager().BanUserAsync(this, "Robot", 86400, "Il nostro robot ha rilevato la pubblicità sull'account. " + GetHabbo().Username, true, false);
            }
            else
            {
                using (IQueryAdapter queryreactor = AkiledEnvironment.GetDatabaseManager().GetQueryReactor())
