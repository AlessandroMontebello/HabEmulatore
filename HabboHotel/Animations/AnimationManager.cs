    ...
    public class AnimationManager
    {
        ...
        private void Cycle()
        {
            ...
            if (this._timer >= this.GetMinutes(START_TIME))
            {
                ...
                string AlertMessage = "<i>Bip bip, è tempo di divertirsi!</i>" +
                "\r\r" +
                "Unisciti a noi su <b>HabHubGame</b> per un gioco chiamato <b>" + Encoding.UTF8.GetString(Encoding.GetEncoding("Windows-1252").GetBytes(room.RoomData.Name)) + "</b>" +
                "\r\r" +
                "Visita la stanza e prova a vincere un premio composto da <i>uno o più RareBox(s) e BadgeBox(s) oltre a un punto per TOP Gamer!</i>" +
                "\r\n" +
                "\r\n- Hanami \r\n";

                AkiledEnvironment.GetGame().GetModerationManager().LogStaffEntry(1953042, "HabHubGames", room.Id, string.Empty, "eventha", string.Format("JeuAuto EventHa: {0}", AlertMessage));
