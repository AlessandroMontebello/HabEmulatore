...
namespace Akiled.HabboHotel.Rooms.Chat.Commands.SpecialPvP
{
    class cachetada : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            if (Params.Length == 1)
            {
                Session.SendWhisper("Per favore scrivi il nome dell'utente che vuoi schiaffeggiare.");
                return;
            }
            ...
            {
                Session.SendWhisper("Vuoi darti uno schiaffo? lol! Stai bene?");
                return;
            }
            ...
            {
                Session.SendWhisper("Questo utente non può essere schiaffeggiato.");
                return;
            }
            ...
            if (!((Math.Abs(TargetUser.X - ThisUser.X) >= 2) || (Math.Abs(TargetUser.Y - ThisUser.Y) >= 2)))
            {
                ...
                Room.SendPacket(new ChatComposer(ThisUser.VirtualId, "@red@ *" + Params[1] + " ha ricevuto uno schiaffo in faccia*", 0, ThisUser.LastBubble));
                Room.SendPacket(new ChatComposer(TargetUser.VirtualId, "@cyan@ Dio, che schiaffo! Cosa ho fatto?", 0, ThisUser.LastBubble));
