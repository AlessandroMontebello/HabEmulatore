...
namespace Akiled.HabboHotel.Rooms.Chat.Commands.Cmd
{
    class VerClones : IChatCommand
    {
        public void Execute(GameClient session, Room Room, RoomUser UserRoom, string[] Params)
        {
            if (Params.Length == 1)
            {
                session.SendWhisper("Inserisci il nome utente da visionare.");
                return;
            }
            ...
            if (AkiledEnvironment.GetGame().GetClientManager().GetClientByUsername(username) != null)
            {
                ...
                using (adapter = AkiledEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    builder.AppendLine("Cloni trovati: " + table.Rows.Count);
            ...
            else
            {
                using (adapter = AkiledEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    ...
                    foreach (DataRow row in table.Rows)
                    {
                        builder.AppendLine(string.Concat(new object[] { "ID : ", row["id"], " - Utente: ", row["username"] }));
                    }
                }
