        ...
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            if (Session.GetHabbo().Rank <= 3)
            {
                if (!((Math.Abs(TargetUser.X - ThisUser.X) >= 2) || (Math.Abs(TargetUser.Y - ThisUser.Y) >= 2)))
                {
                    ...
                    Room.SendPacket(new ChatComposer(ThisUser.VirtualId, "@red@ * Bacia " + Params[1] + " *", 0, ThisUser.LastBubble));
                    System.Threading.Thread.Sleep(500);
                    Room.SendPacketWeb(new PlaySoundComposer("kiss", 2)); //Type = Trax
                    Room.SendPacket(new ChatComposer(TargetUser.VirtualId, "@red@ * Sei stato baciato da :$ *" + Session.GetHabbo().Username + " <3 *", 0, ThisUser.LastBubble));
                    System.Threading.Thread.Sleep(500);
            ...
            {
                Room.SendPacket(new ChatComposer(ThisUser.VirtualId, "@red@ * Super bacia " + Params[1] + " *", 0, ThisUser.LastBubble));
                System.Threading.Thread.Sleep(500);
                Room.SendPacketWeb(new PlaySoundComposer("kiss", 2)); //Type = Trax
                Room.SendPacket(new ChatComposer(TargetUser.VirtualId, "@red@ * Hai ricevuto un super bacio da :$ *" + Session.GetHabbo().Username + " <3 *", 0, ThisUser.LastBubble));
                System.Threading.Thread.Sleep(500);
                ThisUser.ApplyEffect(9);
                System.Threading.Thread.Sleep(500);
                TargetUser.ApplyEffect(9);
