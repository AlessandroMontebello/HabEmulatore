...
namespace Akiled.HabboHotel.Rooms.Chat.Commands.Cmd
{
    class spush : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            if (Params.Length == 1)
            {
                TargetUser.SendWhisperChat("Inserisci il nome dell'utente a cui desideri eseguire il super push.");
                return;
            }

            if (!Room.RoomData.SPushEnabled && !Room.CheckRights(Session, true) && !Session.GetHabbo().HasFuse("room_override_custom_config"))
            {
                Session.SendWhisper("Ops, a quanto pare il proprietario della stanza ha vietato i super push nella sua stanza.");
                return;
            }

            if ((TargetUser.GetClient().GetHabbo().HasFuse("no_accept_use_custom_commands")))
            {
                Session.SendWhisper("Non posso inviare push a questo utente, chiama Hulk.");
                return;
            }
            ...
            if (!((Math.Abs(TargetUser.X - ThisUser.X) >= 2) || (Math.Abs(TargetUser.Y - ThisUser.Y) >= 2)))
            {
                ...
                Room.SendPacket(new ChatComposer(ThisUser.VirtualId, "@red@ * Spingi molto forte " + Params[1] + " *", 0, ThisUser.LastBubble));

            }
            else
            {
                Session.SendWhisper("Oops, " + Params[1] + " non è abbastanza vicino!");
            }
        }
    }
}
