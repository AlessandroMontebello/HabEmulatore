...
namespace Akiled.HabboHotel.Rooms.Chat.Commands.Cmd
{
    class CollectorParkCommand : IChatCommand
    {
        public void Execute(GameClient Session, Room Room, RoomUser UserRoom, string[] Params)
        {
            ...
            {
                Session.SendWhisper("Collector Park è disabilitato!", 1);
                return;
            }

            if (Room.Id != CollectorParkConfigs.roomId)
            {
                Session.SendWhisper("Habitazione no valida!", 1);
                return;
            }
            ...
            if (!string.IsNullOrEmpty(CollectorParkConfigs.badgePass))
            {
                ...
                {
                    Session.SendWhisper("Wow, ti serve il badge per usare Collector Park!", 1);
                    return;
                }
            }
            ...
            Session.SendWhisper("Collector Park attivato!", 34);
        }
    }
}
