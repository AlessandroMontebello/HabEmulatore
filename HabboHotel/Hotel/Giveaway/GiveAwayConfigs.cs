...
namespace AkiledEmulator.HabboHotel.Hotel.Giveaway
{
    public static class GiveAwayConfigs
    {
        ...
        public static void Stop()
        {
            ...
            try
            {
                ...
                if (participants.Count > 0)
                {
                    ...
                    //Message
                    AkiledEnvironment.GetGame().GetClientManager().SendWhisper($"Gli utenti {string.Join(", ", usersWinners)} hanno vinto il giveaway!");
                }
                else
                    AkiledEnvironment.GetGame().GetClientManager().SendWhisper("Non ci sono vincitori!");
        ...
        private static void giveRewards(GameClient client, int credits, int duckets, int diamonds, string badge, int item)
        {
            ...
            //Message
            client.SendMessage(new RoomCustomizedAlertComposer("Congratulazioni, hai vinto il giveaway!" + Environment.NewLine + "Ricompensa: " + Environment.NewLine + $"Crediti: {credits}" + Environment.NewLine + $"Smeraldi: {duckets}" + Environment.NewLine + $"Diamanti: {diamonds}" + Environment.NewLine + $"Badge: {badge}" + Environment.NewLine + $"Articolo: {item}"));
        }
        ...
        public static void Start()
        {
            ...
            AkiledEnvironment.GetGame().GetClientManager().SendMessage(new RoomNotificationComposer($"Nuovo Giveaway aperto! {description}",
                        $"<b><font color='#9200cc'  style='font-size: 16px;'>Descrizione:</font></b>\n<font color='#363636'>{startedByUsername} Ha partecipato al giveaway!\r Usa il comando: ':giveaway' per partecipare </font>\n\n" +
                         $"<b><font color='#9200cc' style='font-size: 16px;'>Premios:</font></b>\n" +
                         $"<b><font color='#b8b814'>Crediti:</font></b> {credits}\n" +
                         $"<b><font color='#00bb92'>Smeraldi:</font></b> {duckets}\n" +
                         $"<b><font color='#008af3'>Diamanti:</font></b> {diamonds}\n" +
                         $"<b><font color='#e36a00'>Badge:</font></b> {(badge ?? "Nessuno")}\n" +
                         $"<b><font color='#a10005'>Item:</font></b> {(ItemData != null ? ItemData.publicname : "Nessuno")}\n" +
                         $"<b><font color='#9200cc'>Termina tra:</font></b> {(timeStart.Minute > 0 ? timeStart.Minute + " Minuti " + timeStart.Second + " Secondi!" : timeStart.Second + " " + " secondo")}!\n" +
                         $"<b><font color='#9200cc'>Creato per :</font></b> {startedByUsername}\n"
                         , giveaway_image, ""));
